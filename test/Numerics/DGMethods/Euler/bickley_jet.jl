...
using CLIMAParameters.Planet: kappa_d, R_d, cp_d, cv_d, T_0, T_surf_ref, MSLP, grav
...
function test_run(
    ...
    #end
    horz_range =
        range(-setup.domain_halflength, length = numelems + 1, stop = setup.domain_halflength)
    vert_range =
        range(-setup.domain_halflength, length = numelems + 1, stop = setup.domain_halflength)
    ...
    model = AtmosModel{FT}(
        ...
        #turbulence = TanhStabilisation(1e-4, 2.0, 1e-3, 1e-4, 0.0),
