...
theta_liquid = 0.1 # Read in from water model {state.θ}
...
m = SoilModel(
    # Define heat capacity of soil
     ρc = (state, aux, t) -> heat_capacity(mineral_properties,porosity,theta_liquid,theta_ice ), # state.θ,state.θi  
    # ρc = (state, aux, t) ->  aux.z > -0.5 ? 2.49e6 : 2.61e6,
    
    # Define thermal conductivity of soil
     κ   = (state, aux, t) ->   thermal_properties(mineral_properties,theta_liquid,theta_ice), # state.θ,state.θi 
