...
"""
Base.@kwdef struct SoilModelMoisture_ice{AbstractParameterSet, Fκ, Fiν, Fsν, Fih, Fiθi} <: BalanceLaw #FiS_l, Fiψ_m, Fiψ, Fih
    "Parameters"
 ...
 # [m3/m3] constant water content in soil
  ...
  # Define initial and boundary condition parameters
  initialθi::Fiθi = (state, aux) -> theta_ice_0 # [m3/m3] constant water content in soil
  #surfaceψ::Fsψ = (state, aux, t) -> 100  #267 # [m3/m3] constant flux at surface
...
vars_state_auxiliary(::SoilModelMoisture_ice, FT) = @vars(z::FT, h::FT,κ::FT) #θl::FT, S_l::FT, ψ_m::FT, ψ::FT,
vars_state_conservative(::SoilModelMoisture_ice, FT) = @vars(ν::FT, θi::FT) #, θi::FT)
vars_state_gradient(::SoilModelMoisture_ice, FT) = @vars(h::FT)
vars_state_gradient_flux(::SoilModelMoisture_ice, FT) = @vars(∇h::SVector{3,FT})

# --------------------------------- 4) CliMA functions needed for simulation -------------------
# ---------------- 4a) Initialization

# Initialize z-Profile ### what role does this play? when?
function init_state_auxiliary!(m::SoilModelMoisture_ice, aux::Vars, geom::LocalGeometry)
  aux.z = geom.coord[3]
...
function init_state_conservative!(m::SoilModelMoisture_ice, state::Vars, aux::Vars, coords, t::Real)
  state.ν = m.initialν(state, aux) #
  state.θi = m.initialθi(state, aux)
end
...
function  soil_nodal_update_aux!(
  ...
  t::Real)
    ...
    aux.κ = hydraulic_conductivity(K_sat,S_l,hydraulic_head(aux.z,ψ), aux.z, "Havercamp")
...
# if state.θi > 0
 ...
 F_T = calculate_frozen_water(state.ν,state.θi,soil_T)
# else
#  F_T = 0;
# end
...
source.ν = -F_T/997 # rho_l = 0.997 # g cm-3, density of water
...
function boundary_state!(nf, m::SoilModelMoisture_ice, state⁺::Vars, aux⁺::Vars,
...
# Boundary condition function
function boundary_state!(nf, m::SoilModelMoisture_ice, state⁺::Vars, diff⁺::Vars,
