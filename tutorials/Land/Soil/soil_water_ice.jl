...
include("soil_water_model_ice.jl")
...
m = SoilModelMoisture_ice(
    ...
    initialθi = (state, aux) -> theta_ice_0 #theta_liq_0, # [m3/m3] constant water content in soil, from Bonan, Ch.8, fig 8.8 as in Haverkamp et al. 1977, p.287,
)
...
export_plot(
    ...
    all_data,
    ("θi",),
    joinpath(output_dir, "foo_ice.png"),
    z_label,
);


open("./ice_final_step.txt", "w") do io
    writedlm(io, all_data[n_outputs])
end

open("./ice_all_time_step.txt", "w") do io
