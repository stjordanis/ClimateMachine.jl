...
Reference: Bonan, Ch.8, fig 8.8 as in Haverkamp et al. 1977, p.287
"""
Base.@kwdef struct SoilModels{Fk, Fiθ, Fsθ, Fih, Fiψ, Fiθi, Fρc, Fκ, FiT, Fst} <: BalanceLaw #, Fsh, Fiψ, Fsψ} <: BalanceLaw
  ...
  K_s::Fk         = (state, aux, t) -> 1e-3#(1e-3*(0.34/(60*60))*1.175e6/((1.175e6+abs.(aux.h-aux.z)^4.74)))
  ...
  # # Define initial and boundary condition parameters
  initialT::FiT = (aux, t) -> 290 # [m3/m3] constant water content in soil
...
vars_state_auxiliary(::SoilModels, FT) = @vars(z::FT, h::FT , ψ::FT, T::FT)
vars_state_conservative(::SoilModels, FT) = @vars(ρcT::FT, θ::FT, θi::FT)
vars_state_gradient(::SoilModels, FT) = @vars(h::FT, T::FT)
vars_state_gradient_flux(::SoilModels, FT) = @vars(∇h::SVector{3,FT}, ∇θ::SVector{3,FT}, ∇T::SVector{3,FT})
